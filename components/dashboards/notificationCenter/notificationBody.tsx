...
function NotificationBody() {
  ...
  return (
    <div className="bg-[#F8FAFC]  text-[0.80rem] h-full w-[100%]">
      <div className="w-[100%] overflow-x-auto">
        <p className=" font-medium font-6 pl-8 pt-6">Notification center</p>
        <div className="flex w-[95%] h-[20%]  mx-auto">
          {content.map((item) => (
            // eslint-disable-next-line react/jsx-key
            <div className="bg-[#6DFFB9] hover:-translate-y-1 hover:cursor-pointer transition ease-in-out delay-2 w-[40%] pt-6 pl-8 pb-6 rounded-xl mt-8 ml-8">
      ...
      <div>
        ...
        <div className="mt-12">
          ...
          <table className="min-w-full divide-y divide-gray-200 border-t-2">
            ...
            <tbody className="divide-y  text-[0.80rem] divide-gray-200 font-inter h-[40vh]">
              {tableData.map((result) => (
                // eslint-disable-next-line react/jsx-key
                <tr>
                  <td className="px-6 py-4 text-[0.80rem] text-sm font-medium text-gray-800 whitespace-nowrap ">
                    <input type="radio" disabled className="mr-3 bg-gray-100" />
                    {result.mine}
                  </td>
                  <td className="px-6 py-6  text-[0.80rem]text-gray-800 whitespace-nowrap flex">
                    {result.date}
                  </td>
                  <td className="px-6 py-4  text-[0.80rem] text-gray-800 whitespace-nowrap">
                    {result.temperature}
                  </td>
                  <td className="px-6 py-4 text-[0.80rem] text-gray-800 whitespace-nowrap">
                    {result.hummidity}
